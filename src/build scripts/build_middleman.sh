...
build_version() {
    ...
    if [ -f "fortran_files.txt" ]; then
        ...
        python -m numpy.f2py -c $FILES -m "$MODULE_NAME" --backend=meson --f77flags="-O3 -ffast-math -funroll-loops" --f90flags="-O3 -ffast-math -funroll-loops"
    else
        echo "fortran_files.txt not found, using all Fortran files..."
        python -m numpy.f2py -c *.f95 *.f *.for -m "$MODULE_NAME" --backend=meson --f77flags="-O3 -ffast-math -funroll-loops" --f90flags="-O3 -ffast-math -funroll-loops"
    fi
